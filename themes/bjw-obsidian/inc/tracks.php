...
function bjw_acf_paired_tracks() {
	...
	foreach ( bjw_rows( 'tandem_tracks', array() ) as $row ) {
		...
		$tracks[] = array(
			'title'  => isset( $row['title'] ) && $row['title'] ? $row['title'] : __( 'Untitled', 'bjw-obsidian' ),
			'artist' => isset( $row['artist'] ) && $row['artist'] ? $row['artist'] : get_bloginfo( 'name' ),
			'a'      => array(
				'label' => isset( $row['a_label'] ) && $row['a_label'] ? $row['a_label'] : __( 'Original', 'bjw-obsidian' ),
				'src'   => $a_src,
			),
			'b'      => array(
				'label' => isset( $row['b_label'] ) && $row['b_label'] ? $row['b_label'] : __( 'Treated', 'bjw-obsidian' ),
				'src'   => $b_src,
			),
		);
	}
